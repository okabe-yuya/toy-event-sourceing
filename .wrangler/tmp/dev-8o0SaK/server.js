...
var Invoice = class {
  ...
  status;
  createdAt;
  constructor(invoiceAmount, invoiceDate) {
...
var Payment = class {
  ...
  status;
  createdAt;
  constructor(targetInvoiceId, paymentAmount) {
...
var Transaction = class {
  ...
  /** 請求を作成して取引を開始する */
  startTransaction(invoiceAmount, invoiceDate) {
  ...
  /** 領収を行う */
  execPayment(targetInvoiceId, paymentAmount) {
    ...
    const totalPaymentAmount = this.payments.reduce(
      (total, payment2) => total + payment2.paymentAmount,
      0
    ) + paymentAmount;
...
// server/events/transaction/startTransactionEvent.ts
var StartTransactionEvent = class {
  constructor(invoiceAmount, invoiceDate) {
    this.invoiceAmount = invoiceAmount;
    this.invoiceDate = invoiceDate;
  ...
  static {
    __name(this, "StartTransactionEvent");
  }
  eventName;
  occurredAt;
  transactionId;
...
var startTransactionEvent_default = StartTransactionEvent;

// server/events/transaction/replay.ts
function replay(events) {
  if (events.length === 0) return null;
  const [headEvent, ...tailEvents] = events;
  if (!(headEvent instanceof startTransactionEvent_default)) {
    throw new Error("\u30A4\u30D9\u30F3\u30C8\u306F\u5FC5\u305A\u53D6\u5F15\u958B\u59CB\u304B\u3089\u958B\u59CB\u3059\u308B\u5FC5\u8981\u304C\u3042\u308A\u307E\u3059");
  }
  let transaction = new Transaction(headEvent.transactionId).startTransaction(headEvent.invoiceAmount, headEvent.invoiceDate);
...
// server/events/transaction/paymentEvent.ts
var PaymentEvent = class {
  constructor(targetInvoiceId, paymentAmount) {
    this.targetInvoiceId = targetInvoiceId;
    this.paymentAmount = paymentAmount;
  ...
  static {
    __name(this, "PaymentEvent");
  }
  eventName;
  occurredAt;
};
var paymentEvent_default = PaymentEvent;

// server/events/transaction/endTransactionEvent.ts
var EndTransactionEvent = class {
  constructor(transactionId) {
    this.transactionId = transactionId;
  ...
  static {
    __name(this, "EndTransactionEvent");
  }
  eventName;
  occurredAt;
};
var endTransactionEvent_default = EndTransactionEvent;

// server/events/transaction/resetTransactionEvent.ts
var ResetTransactionEvent = class {
  constructor(transactionId) {
    this.transactionId = transactionId;
  ...
  static {
    __name(this, "ResetTransactionEvent");
  }
  eventName;
  occurredAt;
};
var resetTransactionEvent_default = ResetTransactionEvent;

// server/server.ts
var app = new Hono2();
var onMemoryStore = /* @__PURE__ */ new Map();
app.get("/api/events", (c) => {
  const transactionId = c.req.query("transactionId");
  const events = onMemoryStore.get(transactionId) ?? [];
  return c.json({ data: events });
});
app.delete("/api/v1/events/transaction", async (c) => {
  const { transactionId } = await c.req.json();
  onMemoryStore.set(transactionId, []);
  return c.json({ data: {} });
});
app.get("/api/v1/transaction/latest", (c) => {
  ...
  if (transactionId) {
    const events = onMemoryStore.get(transactionId) ?? [];
    const result = replay_default(events);
    if (result) {
...
app.get("/api/v1/transaction/replay", (c) => {
  ...
  if (transactionId) {
    ...
    return c.json({ data: replay_default(sliceEvents) });
  }
  return c.json({ data: {} });
});
app.post("/api/v1/events/transactions/startTransactionEvent", async (c) => {
  const { invoiceAmount, invoiceDate } = await c.req.json();
  const event = new startTransactionEvent_default(
  ...
  c.status(201);
  return c.json({ data: replay_default(events) });
});
app.post("/api/v1/events/transactions/paymentEvent", async (c) => {
  const { transactionId, paymentAmount } = await c.req.json();
  if (transactionId) {
    const transactionId_ = transactionId;
    const events = onMemoryStore.get(transactionId_) ?? [];
    const transaction = replay_default(events);
    const latestInvoice = transaction?.latestInvoice();
    if (!latestInvoice) {
      c.status(400);
      return c.json({ message: "\u8ACB\u6C42\u304C\u4F5C\u6210\u3055\u308C\u3066\u3044\u307E\u305B\u3093" });
    }
    const event = new paymentEvent_default(
    ...
    c.status(201);
    return c.json({ data: replay_default(appendEvent) });
  }
  return c.json({ data: {} });
});
app.post("/api/v1/events/transactions/resetTransactionEvent", async (c) => {
  ...
  if (transactionId) {
    const transactionId_ = transactionId;
    const events = onMemoryStore.get(transactionId_) ?? [];
    const event = new resetTransactionEvent_default(transactionId);
    const appendEvent = [...events, event];
    onMemoryStore.set(transactionId_, appendEvent);
    c.status(201);
    return c.json({ data: replay_default(appendEvent) });
...
app.post("/api/v1/events/transactions/endTransactionEvent", async (c) => {
  ...
  if (transactionId) {
    const transactionId_ = transactionId;
    const events = onMemoryStore.get(transactionId_) ?? [];
    const event = new endTransactionEvent_default(transactionId);
    const appendEvent = [...events, event];
    onMemoryStore.set(transactionId_, appendEvent);
    c.status(201);
    return c.json({ data: replay_default(appendEvent) });
...
// .wrangler/tmp/bundle-cia8fc/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-cia8fc/middleware-loader.entry.ts
